...
#Model: IL-6, IFN-g, PAI-1, ANG-1/2
library(dplyr)
library(ggplot2)
...
#Calculate the ratio of ANG1 to ANG2
reserve <- reserve %>%
  mutate(log_ang_ratio = log_ang1 / log_ang2)
...
reserve <- reserve %>%
  mutate(
    log_odds = intercept +
      coef_IL6 * log_il6 +
      coef_IFNg * log_ifn +
      coef_ANG12 * log_ang_ratio +
      coef_PAI1 * log_pai1,
      probability = exp(log_odds) / (1 + exp(log_odds)))

reserve <- reserve %>%
  mutate(phenotype = ifelse(probability > threshold, "reactive", "uninflamed"))

hist(reserve$probability)
...
#Probability of 30d mortality by that of reactive phenotype assignment
library(tidyverse)
reserve_death <- reserve %>% drop_na(death30d, age, sex, illnessduration_enroll, hivrdtresult)

library(ggplot2)
phenotypemodel <- glm(death30d ~ probability + age + sex + illnessduration_enroll + hivrdtresult,
...
reactive_death_prob <- ggplot(reserve_death, aes(x=probability, y=death_prob)) +
  geom_smooth(method = "glm", 
              method.args = list(family = "binomial"), 
              se = TRUE, color = "navy") + xlim(0,1) + ylim(0,1)
reactive_death_prob <- reactive_death_prob + xlab("Probability of reactive phenotype") + ylab("Probability of death at 30 days") + theme_bw()
reactive_death_prob <- reactive_death_prob + theme(text=element_text(size=16)) + annotate("text", x=0.75, y=0.05, label= "aOR 2.90 (95% CI 1.08-7.77)", size = 5)
reactive_death_prob  
...
reserve_phenotype <- reserve %>% select(phenotype)
...
biomarkers$PHENOTYPE <- as.factor(biomarkers$PHENOTYPE)
...
group_assignments = factor(biomarkers$PHENOTYPE, levels=c("uninflamed", "reactive"))
group_assignments
...
contrast.matrix <- makeContrasts(reactivevsuninflamed = reactive - uninflamed, levels = design)
...
results <- topTable(fit2, coef="reactivevsuninflamed", adjust.method="BH", sort.by="P", number=Inf)
...
reserve_model[reserve_model$phenotype=="reactive", "reactive"] <- 1
reserve_model[reserve_model$phenotype=="uninflamed", "reactive"] <- 0
reserve_model$hyper <- as.factor(reserve_model$hyper)

#Clinical + micro
clinicalmicromodel <- glm(reactive ~ age + 
                            sex + 
                            tempmax + 
                       heartrate3 + 
                       resprate3 + 
...
auc(reserve_model$reactive, predicted)

rocobj <- plot.roc(reserve_model$reactive, clinicalmicromodel$fitted.values,
                   percent=FALSE,
                   ci = TRUE,                  
                   print.auc = TRUE,
                   print.auc.x = 0.6, 
...
reserve_model_2[reserve_model_2$phenotype=="reactive", "reactive"] <- 1
reserve_model_2[reserve_model_2$phenotyp=="uninflamed", "reactive"] <- 0
reserve_model_2$hyper <- as.factor(reserve_model_2$hyper)

clinicalmodel <- glm(reactive ~ age + 
                            sex + 
                            tempmax + 
                            heartrate3 + 
                            resprate3 + 
...
auc(reserve_model_2$reactive, predicted)

rocobj <- plot.roc(reserve_model_2$reactive, clinicalmodel$fitted.values,
                   percent=FALSE,
                   ci = TRUE,                  
                   print.auc = TRUE,
                   print.auc.x = 0.6, 
